...
const inputPath = path.join(__dirname, '..', '..', 'data', 'base', 'resumen_base.jsonl');
...
function getSafeCategory(row) {
    return row.category || row.categoria || 'total';
}
...
function buildMetric(current, previous) {
    return {
        val: current,
        var: round(Number(current || 0) - Number(previous || 0))
    };
}
        ...
        output.push({
            ...
            graficos: {
                conversiones_organic_search: [
                    current.conversiones_organic_search
                ],
                trafico_organic_search: [
                    current.trafico_organic_search
                ],
                embudo_conversiones: {
                    impresiones: current.embudo_impresiones,
                    clics: current.embudo_clicks,
                    conversiones: current.embudo_conversiones
                }
            }
        });
    }
