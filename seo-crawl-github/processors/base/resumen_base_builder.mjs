...
const queriesBasePath = path.join(__dirname, '..', '..', 'data', 'base', 'queries_base.jsonl');
const outputPath = path.join(__dirname, '..', '..', 'data', 'base', 'resumen_base.jsonl');
...
function buildMetricsFromUrls(urlsRows) {
    const groupedByMonth = groupBy(urlsRows, row => getMonthRef(row.fecha_ref));
    const metricsByMonth = new Map();

    for (const [fechaRef, rows] of groupedByMonth.entries()) {
        const impressions = sum(rows, 'impressions');
        const clicks = sum(rows, 'clicks');

        metricsByMonth.set(fechaRef, {
...
/* BLOQUE 2: métricas calculadas desde queries_base.jsonl */

function buildMetricsFromQueries(queriesRows) {
    const groupedByMonth = groupBy(queriesRows, row => getMonthRef(row.fecha_ref));
    const metricsByMonth = new Map();
...
function buildResumenBaseRows({
    urlsMetricsByMonth,
    queriesMetricsByMonth
}) {
    ...
    return monthKeys.map(fechaRef => {
        const fallback = buildFallbackMetrics();
        const urlsMetrics = urlsMetricsByMonth.get(fechaRef) || {};
        const queriesMetrics = queriesMetricsByMonth.get(fechaRef) || {};
...
function main() {
    ...
    const queriesRows = readJsonl(queriesBasePath);

    if (!urlsRows.length && !queriesRows.length) {
        console.log('No hay filas para procesar en urls_base.jsonl ni queries_base.jsonl');
        return;
    }

    const urlsMetricsByMonth = buildMetricsFromUrls(urlsRows);
    const queriesMetricsByMonth = buildMetricsFromQueries(queriesRows);

    const resumenRows = buildResumenBaseRows({
        urlsMetricsByMonth,
        queriesMetricsByMonth
    });
